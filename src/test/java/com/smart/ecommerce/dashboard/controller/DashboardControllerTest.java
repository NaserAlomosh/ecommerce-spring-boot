...
class DashboardControllerTest {
    ...
    void adminCanLoadDashboard() throws Exception {
        when(messages.getMessage("dashboard.loaded")).thenReturn("Dashboard overview loaded successfully");
    ...
    void subAdminIsForbiddenByMethodSecurity() throws Exception {
        mvc.perform(get("/api/v1/admin/dashboard").with(user("sub").roles("SUB_ADMIN")))
                .andExpect(status().isForbidden());
    }

    private DashboardResponse emptyResponse() {
        return new DashboardResponse(new SalesSummary(BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO), new OrderSummary(0,0,0,0,0,0,0), new ProductSummary(0,0,0), new CategorySummary(0), new CustomerSummary(0), new InventorySummary(0,0), new ReviewSummary(0, BigDecimal.ZERO), List.of(), List.of(), List.of(), List.of());
    }
