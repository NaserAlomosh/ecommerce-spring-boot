...
public interface UserRepository
  ...
  @Query("select u from User u where (:q is null or " +
         "lower(concat(u.firstName,' ',u.lastName,' ',u.email,' " +
         "',u.phoneNumber)) like lower(concat('%',:q,'%'))) and (:role is " +
         "null or u.role=:role) and (:status is null or u.status=:status)")
  Page<User>
  search(@Param("q") String q, @Param("role") Role role,
