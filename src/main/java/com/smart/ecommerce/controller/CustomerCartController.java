...
public class CustomerCartController {
  ...
  public ApiResponse<CartResponse> get() {
    return ApiResponse.success("Cart", service.get());
  }
  @PostMapping("/items")
  public ApiResponse<CartResponse>
  add(@Valid @RequestBody AddCartItemRequest r) {
    return ApiResponse.success("Cart item added", service.add(r));
  }
  @PatchMapping("/items/{cartItemId}")
  public ApiResponse<CartResponse>
  update(@PathVariable Long cartItemId,
         @Valid @RequestBody UpdateCartItemQuantityRequest r) {
    return ApiResponse.success("Cart item updated",
                               service.update(cartItemId, r));
  }
  @DeleteMapping("/items/{cartItemId}")
  public ApiResponse<Void> remove(@PathVariable Long cartItemId) {
    service.remove(cartItemId);
