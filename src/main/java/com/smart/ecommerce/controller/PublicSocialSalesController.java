...
@RequestMapping("/api/v1/social-sales-link")
public class PublicSocialSalesController {
  private final WebsiteService websiteService;
  private final OrderService orderService;
  private final MessageUtil messages;

  @GetMapping
  public ApiResponse<SocialSalesLinkResponse> link() {
    return ApiResponse.success(messages.getMessage("social_sales_link.loaded"),
                               websiteService.socialSalesLink());
  }
  ...
  public ApiResponse<OrderResponse> createOrder(
    ...
    return ApiResponse.success(messages.getMessage("order.created"),
                               orderService.createPublic(request));
  }
}
