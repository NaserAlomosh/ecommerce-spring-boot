...
public final class UserDtos {
  ...
  public record UserResponse(Long id, String firstName, String lastName,
                             ...
                             boolean phoneVerified, String profileImage,
                             Instant lastLoginAt, Instant createdAt,
                             Instant updatedAt) {}

  public record
  UpdateProfileRequest(@NotBlank @Size(min = 2, max = 80) String firstName,
  ...
  public record AdminCreateUserRequest(
      ...
      @NotBlank @Pattern(regexp = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d).{8,72}$")
      String password, @NotNull Role role) {}

  public record StatusRequest(@NotNull UserStatus status) {}

  public record AdminChangePasswordRequest(@NotBlank @Pattern(
      regexp = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d).{8,72}$") String newPassword) {
  }
}
