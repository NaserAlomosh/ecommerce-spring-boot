...
public class WishlistService {
 ...
 private WishlistProductResponse toProduct(Product p){ BigDecimal eff=p.getDiscountPrice()==null?p.getPrice():p.getDiscountPrice(); BigDecimal pct=p.getDiscountPrice()==null?BigDecimal.ZERO:p.getPrice().subtract(p.getDiscountPrice()).multiply(BigDecimal.valueOf(100)).divide(p.getPrice(),2, RoundingMode.HALF_UP); return new WishlistProductResponse(p.getId(), p.getCategory().getId(), p.getNameEn(), p.getNameAr(), p.getSku(), p.getPrice(), p.getCurrency(), p.getDiscountPrice(), eff, pct, primary(p), p.isActive(), p.getStockQuantity()>0, p.getStockQuantity()); }
