...
public class ProductService {
    ...
    public ProductResponse create(ProductCreateRequest request, List<MultipartFile> images) {
        ...
        try {
            ...
            map(request, product);
            product.setCategory(categoryRepository.findById(request.categoryId()).orElseThrow(() -> new ResourceNotFoundException("Category not found")));
    ...
    private ProductResponse toResponse(Product p) { BigDecimal eff = p.getDiscountPrice() == null ? p.getPrice() : p.getDiscountPrice(); BigDecimal pct = p.getDiscountPrice() == null ? BigDecimal.ZERO : p.getPrice().subtract(p.getDiscountPrice()).multiply(BigDecimal.valueOf(100)).divide(p.getPrice(), 2, RoundingMode.HALF_UP); return new ProductResponse(p.getId(), p.getCategory().getId(), p.getNameEn(), p.getNameAr(), p.getSku(), p.getPrice(), p.getDiscountPrice(), eff, pct, p.getStockQuantity(), p.getLowStockThreshold(), p.getStockQuantity() > 0, p.getStockQuantity() <= p.getLowStockThreshold(), p.isActive(), p.isFeatured(), p.getImages().stream().map(i -> new ProductImageResponse(i.getId(), i.getImageUrl(), i.isPrimaryImage(), i.getSortOrder())).toList()); }
}
