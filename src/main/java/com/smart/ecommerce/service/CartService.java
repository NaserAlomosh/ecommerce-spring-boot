...
public class CartService {
  ...
  public CartResponse add(AddCartItemRequest r) {
    User u = ctx.currentCustomer();
    Product p =
        products.lockWithImagesById(r.productId())
            .orElseThrow(
  ...
  @Transactional
  public CartResponse update(Long id, UpdateCartItemQuantityRequest r) {
    CartItem it = owned(id);
    Product p =
        products.lockWithImagesById(it.getProduct().getId())
            .orElseThrow(
                () -> new ResourceNotFoundException("Product not found"));
    validateAvailable(p);
    validateQty(r.quantity(), p);
  ...
  public void clear() {
    ...
    carts.lockByCustomerIdAndStatus(u.getId(), CartStatus.ACTIVE)
        .ifPresent(c -> items.deleteByCartId(c.getId()));
  }
  private CartItem owned(Long id) {
    User u = ctx.currentCustomer();
    CartItem it = items.findById(id).orElseThrow(
  ...
  private void validateAvailable(Product p) {
    if (p.isDeleted())
  ...
  private void validateQty(int q, Product p) {
    if (q < 1)
  ...
  private CartResponse toResponse(Cart c) {
    List<CartItemResponse> rs =
        c.getItems()
            ...
            .map(this::toItem)
            .toList();
    String currency = cartCurrency(rs);
    BigDecimal sub = rs.stream()
                         .map(CartItemResponse::lineTotal)
                         .reduce(BigDecimal.ZERO, BigDecimal::add);
    int total = rs.stream().mapToInt(CartItemResponse::quantity).sum();
    return new CartResponse(c.getId(), c.getStatus(), rs, total, rs.size(), sub,
                            currency);
  }
  private CartItemResponse toItem(CartItem it) {
    Product p = it.getProduct();
    BigDecimal eff =
        p.getDiscountPrice() == null ? p.getPrice() : p.getDiscountPrice();
    BigDecimal line = eff.multiply(BigDecimal.valueOf(it.getQuantity()));
    boolean inStock = p.getStockQuantity() > 0;
    boolean avail = !p.isDeleted() && p.isActive() && inStock;
    return new CartItemResponse(
        it.getId(),
        new CartProductResponse(p.getId(), p.getNameEn(), p.getNameAr(),
                                p.getSku(), primary(p), p.isActive(), inStock,
                                p.getStockQuantity(), p.getAverageRating(),
                                p.getReviewsCount()),
        it.getQuantity(), p.getPrice(), p.getCurrency(), p.getDiscountPrice(),
        eff, line, avail, it.getQuantity() > p.getStockQuantity(),
        it.getCreatedAt(), it.getUpdatedAt());
  }
  private String cartCurrency(List<CartItemResponse> rs) {
    String currency = CurrencyCode.DEFAULT_CURRENCY;
    for (CartItemResponse r : rs) {
      if (!currency.equals(r.currency()))
        throw new IllegalArgumentException(
            "Mixed currencies in cart are not supported");
    }
    return currency;
  }
