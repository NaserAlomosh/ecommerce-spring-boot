...
public class CartService {
 ...
 private CartItemResponse toItem(CartItem it){ Product p=it.getProduct(); BigDecimal eff=p.getDiscountPrice()==null?p.getPrice():p.getDiscountPrice(); BigDecimal line=eff.multiply(BigDecimal.valueOf(it.getQuantity())); boolean inStock=p.getStockQuantity()>0; boolean avail=!p.isDeleted()&&p.isActive()&&inStock; return new CartItemResponse(it.getId(), new CartProductResponse(p.getId(),p.getNameEn(),p.getNameAr(),p.getSku(),primary(p),p.isActive(),inStock,p.getStockQuantity(),p.getAverageRating(),p.getReviewsCount()), it.getQuantity(), p.getPrice(), p.getCurrency(), p.getDiscountPrice(), eff, line, avail, it.getQuantity()>p.getStockQuantity(), it.getCreatedAt(), it.getUpdatedAt()); }
