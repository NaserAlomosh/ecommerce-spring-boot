...
public class CartService {
 ...
 private CartResponse toResponse(Cart c){ List<CartItemResponse> rs=c.getItems().stream().sorted(Comparator.comparing(CartItem::getCreatedAt, Comparator.nullsLast(Comparator.naturalOrder()))).map(this::toItem).toList(); BigDecimal sub=rs.stream().map(CartItemResponse::lineTotal).reduce(BigDecimal.ZERO, BigDecimal::add); int total=rs.stream().mapToInt(CartItemResponse::quantity).sum(); return new CartResponse(c.getId(), c.getStatus(), rs, total, rs.size(), sub); }
 private CartItemResponse toItem(CartItem it){ Product p=it.getProduct(); BigDecimal eff=p.getDiscountPrice()==null?p.getPrice():p.getDiscountPrice(); BigDecimal line=eff.multiply(BigDecimal.valueOf(it.getQuantity())); boolean inStock=p.getStockQuantity()>0; boolean avail=!p.isDeleted()&&p.isActive()&&inStock; return new CartItemResponse(it.getId(), new CartProductResponse(p.getId(),p.getNameEn(),p.getNameAr(),p.getSku(),primary(p),p.isActive(),inStock,p.getStockQuantity()), it.getQuantity(), p.getPrice(), p.getDiscountPrice(), eff, line, avail, it.getQuantity()>p.getStockQuantity(), it.getCreatedAt(), it.getUpdatedAt()); }
