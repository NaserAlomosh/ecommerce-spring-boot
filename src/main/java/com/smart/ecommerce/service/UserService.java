...
public class UserService {
  ...
  public UserResponse me(String email) {
    return mapper.toResponse(current(email));
  }

  @Transactional
  public UserResponse updateMe(String email, UpdateProfileRequest r) {
    User u = current(email);
    if (!u.getPhoneNumber().equals(r.phoneNumber()) &&
        users.existsByPhoneNumber(r.phoneNumber()))
      throw new IllegalArgumentException("error.phone_exists");
    u.setFirstName(r.firstName());
    u.setLastName(r.lastName());
    u.setPhoneNumber(r.phoneNumber());
    u.setProfileImage(r.profileImage());
  ...
  public Page<UserResponse> search(String q, Role role, UserStatus status,
                                   Pageable p) {
    return users.search(q, role, status, p).map(mapper::toResponse);
  }
  ...
  public UserResponse status(String actorEmail, Long id, StatusRequest r) {
    ...
    requireCanManageTarget(actor, u);
    if (u.getRole() == Role.ADMIN && u.getStatus() == UserStatus.ACTIVE &&
        r.status() != UserStatus.ACTIVE &&
        users.countByRoleAndStatus(Role.ADMIN, UserStatus.ACTIVE) <= 1)
      throw new AccessDeniedException("error.last_admin");
    u.setStatus(r.status());
    if (r.status() == UserStatus.BLOCKED)
      refreshTokens.revokeAll(u, java.time.Instant.now());
    return mapper.toResponse(u);
  }

  @Transactional
  public void adminChangePassword(String actorEmail, Long id,
                                  AdminChangePasswordRequest r) {
    User actor = current(actorEmail);
    User target = users.findById(id).orElseThrow();
    requireCanManageTarget(actor, target);
    if (actor.getRole() != Role.ADMIN)
      throw new AccessDeniedException("error.forbidden");
    target.setPasswordHash(encoder.encode(r.newPassword()));
    refreshTokens.revokeAll(target, java.time.Instant.now());
  }
  ...
  public void delete(String actorEmail, Long id) {
    ...
    if (target.getId().equals(actor.getId()))
      throw new AccessDeniedException("error.delete_self");
    if (target.getRole() == Role.ADMIN &&
        users.countByRoleAndStatus(Role.ADMIN, UserStatus.ACTIVE) <= 1)
      throw new AccessDeniedException("error.last_admin");
    refreshTokens.revokeAll(target, java.time.Instant.now());
    target.setStatus(UserStatus.DELETED);
    target.setEmail("deleted-" + target.getId() + "-" + target.getEmail());
    target.setPhoneNumber(
        "+999" + String.format("%012d", target.getId() % 1_000_000_000_000L));
  }

  private void requireCanManageTarget(User actor, User target) {
    requireCanManageRole(actor, target.getRole());
  }
  private void requireCanManageRole(User actor, Role role) {
    if (actor.getRole() == Role.ADMIN)
      return;
    if (actor.getRole() == Role.SUB_ADMIN &&
        (role == Role.CUSTOMER || role == Role.DELIVERY))
      return;
    throw new AccessDeniedException("error.forbidden");
  }
}
