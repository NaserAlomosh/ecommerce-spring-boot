...
public class OrderStatusTransitionService {
  private static final Map<OrderStatus, Set<OrderStatus>> ALLOWED = Map.of(
      ...
      Set.of(OrderStatus.COMPLETED, OrderStatus.FAILED), OrderStatus.FAILED,
      Set.of(OrderStatus.PROCESSING, OrderStatus.CANCELLED),
      OrderStatus.COMPLETED, Set.of(), OrderStatus.CANCELLED, Set.of());
  public void validate(OrderStatus from, OrderStatus to) {
    if (!ALLOWED.getOrDefault(from, Set.of()).contains(to))
      throw new IllegalArgumentException("order.error.invalid_transition");
  }
  public boolean canAdminCancel(OrderStatus status) {
