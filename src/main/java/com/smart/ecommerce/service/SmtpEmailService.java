...
public class SmtpEmailService implements EmailService {
    ...
    @Value("${spring.mail.retry.max-attempts:2}")
    private int maxAttempts;

    @Value("${spring.mail.retry.backoff-millis:1000}")
        ...
        for (int attempt = 1; attempt <= attempts; attempt++) {
            try {
                ...
                if (!isTransientReadTimeout(ex) || attempt == attempts) {
                    log.error(
                            "Failed to send email '{}' to {} after {}/{} attempt(s). Check SMTP_HOST, SMTP_PORT, SMTP_USERNAME, SMTP_PASSWORD, SMTP_FROM, SMTP_TLS, SMTP_TIMEOUT, and SMTP_WRITE_TIMEOUT.",
                            subject,
                            to,
                            attempt,
                            attempts,
                ...
                log.warn(
                        "Transient SMTP read timeout while sending email '{}' to {} on attempt {}/{}. Retrying after {} ms.",
                        subject,
                        to,
                        attempt,
                        attempts,
    ...
    private void sendOnce(String to, String subject, String html) throws Exception {
