...
The compatibility DTOs `GuestLinkRequest`, `GuestLinkResponse`, and
`GuestLinkStatusRequest` are available in `GuestOrderDtos` for code that imports
them. The active single-link admin contract remains implemented by
`AdminWebsiteController` and uses `GuestOrderLinkResponse` and
`GuestOrderLinkUpdateRequest` from `WebsiteDtos`. The guest checkout request is
`GuestOrderRequest` from `GuestOrderDtos`.
